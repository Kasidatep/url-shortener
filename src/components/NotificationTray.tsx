...
export function NotificationProvider({children}:{children:React.ReactNode}){
  ...
  return <Context.Provider value={value}>{children}<aside className="notification-tray" aria-live="polite" aria-label="Notifications">{notices.map(notice=><div className={'notice-toast '+notice.tone} key={notice.id}><span className="notice-icon" aria-hidden="true">{notice.tone==='success'?'✓':notice.tone==='error'?'!':'↗'}</span><p>{notice.message}</p><button aria-label="Dismiss" onClick={()=>dismiss(notice.id)}>×</button><i/></div>)}</aside></Context.Provider>;
}
