...
export default function FaqPage() {
  ...
  return <main>
    ...
    <section className="help-sections">
      {text.faqSections.map(([title, items]) => <section className="help-section" key={title}><h2>{title}</h2><div>{items.map(([question, answer]) => <details key={question}><summary>{question}</summary><p>{answer}</p></details>)}</div></section>)}
    </section>
  </main>;
}
